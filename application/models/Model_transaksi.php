...
class Model_transaksi extends CI_Model
{
   ...
   {
      $query = "SELECT `tb_pesanan_detail`.*, `tb_barang`.`nama_barang` FROM `tb_pesanan_detail` JOIN `tb_barang` ON `tb_pesanan_detail`.`id_barang`=`tb_barang`.`id_barang` WHERE `tb_pesanan_detail`.`id_pesanan`=$last_id";
   ...
   public function barang_invoice($id_invoice)
   {
      ...
      $query = "SELECT `tb_pesanan_detail`.*, `tb_barang`.`nama_barang` FROM `tb_pesanan_detail` JOIN `tb_barang` ON `tb_pesanan_detail`.`id_barang`=`tb_barang`.`id_barang` JOIN `invoice` ON `invoice`.`id_invoice`=`tb_pesanan_detail`.`id_invoice` WHERE `tb_pesanan_detail`.`id_invoice`=$id_invoice";
   ...
   public function get_detail_pesanan($id_invoice)
   {
      ...
      $query = "SELECT `tb_pesanan`.`nama_penerima`, `tb_pesanan`.`telepon`, `tb_pesanan`.`alamat`, `tb_pesanan`.`provinsi`, `tb_pesanan`.`distrik`, `tb_pesanan`.`type`, `tb_pesanan`.`kodepos`, `tb_pesanan`.`catatan`, `invoice`.`tanggal_pesan`  
