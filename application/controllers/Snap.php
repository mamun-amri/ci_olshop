...
class Snap extends CI_Controller
{
	...
	{
		parent::__construct();
		$params = array('server_key' => 'SB-Mid-server-ZhM0PV_-G9bgzuK0e9xEF-Js', 'production' => false);
	...
	public function token()
	{
		...
		$this->db->select("
			...
			tb_barang.harga as price,
			tb_barang.nama_barang as name
		");
		$this->db->from('tb_pesanan_detail');
		$this->db->join('tb_barang', 'tb_barang.id_barang=tb_pesanan_detail.id_barang');
		...
		$billing_address = array(
			'first_name'    => "SATE BANDENG",
			'last_name'     => "HJ. MARIYAM",
			'address'       => "Jl.xx xx",
			'city'          => "Serang",
			'postal_code'   => "00000",
			'phone'         => "081122334455",
	...
	{
		$data['title'] = 'Status Transaksi Edit';
		$data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
		$data['status'] = $this->db->get_where('tb_transaksi', ['id_transaksi' => $id_transaksi])->row_array();
		$this->load->view('admin_templates/header', $data);
		$this->load->view('admin_templates/sidebar');
		$this->load->view('admin_templates/topbar', $data);
	...
	public function edit_proses()
	{
		...
		$this->db->query("update tb_transaksi set transaction_status='" . $status . "' where id_transaksi='" . $id_transaksi . "'");
	...
	public function konfirmasi_transaksi()
	{
		...
		$this->db->query("update tb_transaksi set transaction_status='" . 'Barang Telah di Terima' . "'where id_transaksi='" . $id_transaksi . "'");
		$this->detail($id_pesanan);
	}
}
