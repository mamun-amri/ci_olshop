...
class Transaction extends CI_Controller
{
	...
	public function status($order_id)
	{
		...
		if ($status['transaction_status'] == 'expired') {
			...
			$result = $this->db->select('tb_pesanan_detail.*, tb_barang.stok')->from('tb_pesanan_detail')->join('tb_barang', 'tb_barang.id_barang=tb_pesanan_detail.id_barang')->where('id_pesanan', $id_pesanan)->get()->result_array();
			foreach ($result as $row) {
				$id_barang = $row['id_barang'];
				$stok = (int)$row['stok'];
				$qty = (int)$row['qty'];
				$sisa = $stok - $qty;
				$this->db->query("update tb_barang set stok='" . $sisa . "' where id_barang='" . $id_barang . "'");
			}
		}
