<div class="container m-b-50 m-t-100" style="font-size: 13px;">
   <div class="row mt-4">
      <div class="col-md-6">
         <!-- kolom barang -->
         ...
         <a class="btn btn-sm btn-danger mt-4 shadow" href="<?= base_url('snap/status_transaksi') ?>"><i class="fas fa-backspace"></i> Kembali</a>
         <form action="<?= base_url('snap/konfirmasi_transaksi') ?>" method="POST">
            <input type="hidden" name="id" id="id" value="<?= $detail_pelanggan['id_transaksi'] ?>">
            <input type="hidden" name="id_pesanan" id="id_pesanan" value="<?= $detail_pelanggan['id_pesanan'] ?>">

            <?php if ($detail_pelanggan['transaction_status'] == 'expire') : ?>
               <button type="submit" class="btn btn-sm btn-success mt-3 shadow" disabled>Konfirmasi Barang di Terima</button>
            <?php elseif ($detail_pelanggan['transaction_status'] == 'Barang Telah di Terima') : ?>
               <button type="submit" class="btn btn-sm btn-success mt-3 shadow" disabled>Konfirmasi Barang di Terima</button>
            <?php else : ?>
               <button type="submit" class="btn btn-sm btn-success mt-3 shadow">Konfirmasi Barang di Terima</button>
            <?php endif; ?>
         </form>
